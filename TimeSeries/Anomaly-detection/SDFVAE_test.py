...
import numpy as np
import math
from model.SDFVAE import *
from tqdm import *
from data_preprocessing.SDFVAE_pre import KpiReader
from utils.SDFVAE_utils import Logger


class Tester(object):
    def __init__(self, model, device, test, testloader, log_path='log_tester', log_file='loss', learning_rate=0.0002,
                 checkpoints=None):
        self.model = model
    ...
    def model_test(self):
        ...
        for i, dataitem in enumerate(self.testloader, 1):
            ...
            s_mean, s_logvar, s, d_post_mean, d_post_logvar, d, d_prior_mean, d_prior_logvar, recon_x_mu, recon_x_logsigma = self.forward_test(
                data)
    ...
    def forward_test(self, data):
        with torch.no_grad():
            s_mean, s_logvar, s, d_post_mean, d_post_logvar, d, d_prior_mean, d_prior_logvar, recon_x_mu, recon_x_logsigma = self.model(
                data)
            return s_mean, s_logvar, s, d_post_mean, d_post_logvar, d, d_prior_mean, d_prior_logvar, recon_x_mu, recon_x_logsigma

    def loss_fn(self, original_seq, recon_seq_mu, recon_seq_logsigma, s_mean, s_logvar, d_post_mean, d_post_logvar,
                d_prior_mean, d_prior_logvar):
        ...
        kld_d = 0.5 * torch.sum(d_prior_logvar - d_post_logvar +
                                ...
                                - 1)
        return (
                           -loglikelihood + kld_s + kld_d) / batch_size, loglikelihood / batch_size, kld_s / batch_size, kld_d / batch_size
...
def main():
    ...
    # Dataset options
    parser.add_argument('--dataset_path', type=str, default='./datasets/machine_data/machine-1-1-train')
    ...
    parser.add_argument('--checkpoints_path', type=str, default='results/sdfvae_machine-1-1')
    parser.add_argument('--checkpoints_file', type=str, default='')
    parser.add_argument('--checkpoints_interval', type=int, default=10)
    parser.add_argument('--log_path', type=str, default='./results/sdfvae_log_tester')
